     ...
     register struct buffer *b;
{
  ...
  for (offset = (char *)&buffer_local_flags.name - (char *)&buffer_local_flags;
    ...
    if (*(long *)(offset + (char *) &buffer_local_flags) > 0
	|| *(long *)(offset + (char *) &buffer_local_flags) == -2)
     ...
     register Lisp_Object buffer;
{
  ...
  /* Add on all the variables stored in special slots.  */
  {
    ...
    for (offset = (char *)&buffer_local_symbols.name - (char *)&buffer_local_symbols;
	 ...
	 offset += (sizeof (Lisp_Object)))
      {
	mask = *(long *)(offset + (char *) &buffer_local_flags);
...
init_buffer_once ()
{
  ...
  if (sizeof (long) != sizeof (Lisp_Object)) abort ();
